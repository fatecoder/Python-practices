...
def input_usuario():
    if espacios() is True:
        coor = raw_input("Ingresa coordenadas (X,Y): ")
        if len(coor) == 3 and coor[1] == ',' and coor[0].isdigit() is True and coor[2].isdigit() is True:
            cx = int(coor[0])
            cy = int(coor[2])
            ...
            print "Formato incorrecto. Formato: {digito}{,}{digito}"
            input_usuario()
