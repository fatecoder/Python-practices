...
def articlesToFile(my_car, sto, fil):
    ...
    for el in my_car.articles:
        f.write("%s %i $%i\n" %(el,my_car.articles[el],sto.articles_stock[el][0]))
...
def updateFile(sto, fil):
    ...
    for el in sto.articles_stock:
        if i != len(sto.articles_stock):
            f.write("%s,%i,%i\n" %(el,sto.articles_stock[el][0],sto.articles_stock[el][0]))
        else:
            f.write("%s,%i,%i" %(el,sto.articles_stock[el][0],sto.articles_stock[el][1]))
        i = i + 1
    f.close()
...
def buy(my_car, sto):
    ...
    if ans.isdigit() :
        if int(ans) is 0:
            ...
            cant = raw_input("How many?: ")
            if cant.isdigit() and sto.check_articles_stock(art, int(cant)):
                my_car.add_article(art, int(cant))
            else:
            ...
            art = raw_input("Choose an article: ")
            cant = raw_input("How many?: ")
            if cant.isdigit() and art in my_car. articles and int(cant) <= my_car.articles[art] and int(cant) > 0:
                sto.return_to_stock(art, int(cant))
                my_car.rem_article(art, int(cant))
            else:
                print "Error trying remove"
            buy(my_car, sto)
        elif int(ans) is 3:
